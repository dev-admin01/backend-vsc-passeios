...
exports.RunMigrationsService = void 0;
const child_process_1 = require("child_process");
class RunMigrationsService {
    async execute() {
        return new Promise((resolve, reject) => {
            // 1) Primeiro spawn: "migrate deploy"
            const prismaMigrate = (0, child_process_1.spawn)("npx", ["prisma", "migrate", "deploy"], {
                shell: true,
            ...
            prismaMigrate.on("close", code => {
                if (code !== 0) {
                    return reject(new Error(`Migrações falharam com código ${code}\nLog: ${migrateOutput}`));
                }
                // 2) Se chegou aqui, "migrate deploy" rodou com sucesso.
                // Agora vamos rodar o seed
                const prismaSeed = (0, child_process_1.spawn)("npx", ["prisma", "db", "seed"], {
                    shell: true,
                    stdio: ["pipe", "pipe", "pipe"],
                });
                let seedOutput = "";
                ...
                prismaSeed.on("close", seedCode => {
                    if (seedCode !== 0) {
                        return reject(new Error(`Seed falhou com código ${seedCode}\nLog: ${seedOutput}`));
                    }
                    ...
                    const endIndex = migrateOutput.indexOf("All migrations have been successfully applied.");
                    // Se não achar, usa o output inteiro
                    const migrationsBlock = startIndex !== -1 && endIndex !== -1
...
exports.RunMigrationsService = RunMigrationsService;
